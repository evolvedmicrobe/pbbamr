...
ind = loadpbi(bamname)
# Show the first 3 rows (kable just formats)
knitr::kable(ind[1:3,])
```
...
The PacBio index table can tell you a lot about the data.  A few examples follow. 
...
# the preceding and deleted base
findSingleDeletions <- function(aln) {
  ...
  # And now let's grab the preceding and deleted base
